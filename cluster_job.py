...
'q':'short-sl7,rg-el7', 'bin':'~mmariotti/bin_new_cluster/', 'email':'marco.mariotti@crg.eu', 'E':'a', 'e':0, 'f':0, 'x':0,
'q_syn':'S=short-sl7,rg-el7;L=long-sl7,rg-el7' }
...
############ loading options
  ...
  if input_file=='-':  input_file_h=sys.stdin
    ...
    if type(input_file) == int: raise notracebackException, "ERROR you must specify an input file. Usage: cluster_job.py -i INPUTFILE [other options and arguments]"
    check_file_presence(input_file, 'input_file', notracebackException)
    input_file_h=open(input_file)

  if opt['email']=='marco.mariotti@crg.eu' and bbash('whoami')!='mmariotti' and opt['E']: raise notracebackException, not_my_email_err
...
#  single_job_mode=False
  if tot_lines==0:    raise notracebackException, "ERROR input file is empty (or all lines are commented)!" 
  if tot_lines==1:    
  ...
  if is_directory(output_folder):  
    if not opt['f'] and sys.stdout.isatty(): 
